...
function CreateUserPage() {
  ...
  const [hasClickedTermsLink, setHasClickedTermsLink] = useState(false);
  const [hasAgreedToTerms, setHasAgreedToTerms] = useState(false);
  const [termsError, setTermsError] = useState('Please verify you have read and agree to the terms and conditions');
  ...
  const handleSubmit = async (event) => {
    ...
    if (!hasClickedTermsLink || !hasAgreedToTerms) {
      setTermsError('Please click the Terms and Conditions link and check the box to agree.');
      return;
    }
    ...
    try {
      const response = await axios.post(REACT_APP_BASEURL + '/user/create', userData, {
        headers: {
      ...
      if (response.data.success) {
        setMessage(`User ${username} added successfully! Redirecting...`);
