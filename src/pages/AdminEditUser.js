...
const AdminEditUser = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${REACT_APP_BASEURL}/admin/user/${userId}`, {
          params: { admin_id: storedUser.id }
        });

        if (response.data.success) {
          const { user, driverInfo } = response.data;
          ...
          if (user.Username) {
            setPreviewUrl(`${REACT_APP_BASEURL}/user/profile-picture/${user.Username}`);
          }
        } else {
          setError('Error fetching user information: ' + response.data.message);
        }
      } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    // Check if the field belongs to userData
    ...
    // Check if the field belongs to driverData
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${REACT_APP_BASEURL}/admin/update-user/${userId}/${userType}`, requestData);
      ...
      if (response.data.success) {
        ...
        setError('Error updating user information: ' + response.data.message);
      }
    } catch (error) {
